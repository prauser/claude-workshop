...
## 7. 적용 결정사항 (2026-04-09)

ECC 분석을 기반으로 claude-workshop에 적용한/하지 않기로 한 항목 정리.

### 적용 완료

| 항목 | 적용 방식 | 비고 |
|------|----------|------|
| **Rules 체계** | `.claude/rules/`로 모듈 분리 | CLAUDE.md는 프로젝트 설명만, 행동 규칙은 rules/로 |
| **Iterative Retrieval** | spec-plan의 Code/Spec Agent에 3사이클 검색 루프 | DISPATCH→EVALUATE→REFINE→LOOP |
| **templates/ 활성화** | `templates/impl-workflow/`에 Hook 스크립트, settings.json, Config 예시 | graduation flow 빈 단계 채움 |
| **Verification (부분)** | impl에서 reviewer↔implementer 최대 3회 반복 | ECC의 pass@k보다 단순하지만 실용적 |

### Improver로 대체 (별도 구현 불필요)

| ECC 기능 | 대체 방식 | 이유 |
|----------|----------|------|
| **Continuous Learning (Background Observer)** | Improver가 session jsonl 후처리 | 실시간 Hook 관측 불필요. 비용/품질 리스크 회피 |
| **/save-session** | Improver 자동 분석 + 대시보드 승인 | 수동 호출의 편향/누락 문제 해소 |
| **/learn-eval** | Improver 자동 패턴 추출 + 인간 승인 | 동일 |
| **Skill Evolution** | Improver가 제안, 인간이 승인 | 자동 롤백/승격은 위험 |

### 보류

| 항목 | 이유 | 재검토 시점 |
|------|------|------------|
| **Verification Loops (eval 기준 정의)** | 실사용 후 필요성 판단 | 워크플로우 실사용 시작 후 |
| **Context Budget 감사** | md-reviewer가 부분 커버 | 토큰 비용 문제 발생 시 |
| **Strategic Compact** | 개인 수준, 조직 강제 불필요 | 필요 시 개인 설정으로 |
| **Worktree 병렬 오케스트레이션** | 현재 spec-plan 4개 병렬로 충분 | 태스크 규모 증가 시 |

### UE 테스트 전략 (별도 문서)

- LLT → implementer TDD (CLAUDE.md에서 프로젝트별 지시)
- Automation Test → PR Hook / integrator
- Gauntlet → CI/CD (워크플로우 밖)
- 상세: `notes/impl-workflow/ue-test-strategy.md`
